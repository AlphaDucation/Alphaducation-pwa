(function () {
  ...
  function inferSeriesCode(series, explicitName) {
  ...
  const seriesCode = inferSeriesCode(data.series, seriesName);
  ...
  setText("meta-series", seriesName + " \u00b7 Series " + seriesCode);
  ...
  if (toolUrlNode && data.toolbox) {
    const url = normalizeText(data.toolbox.url);
    if (url) {
      toolUrlNode.href = url;
      toolUrlNode.textContent = url;
    }
  }
